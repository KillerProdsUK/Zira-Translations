...
module.exports = {
  ...
  help: [{
    name: 'Команды',
    value: '**z/role message** ~~-~~ сообщение реагировать на\n**z/role channel** ~~-~~ канал для реакции\n**z/role add** ~~-~~ Добавление роли с emoji\n**z/role del** ~~-~~ Удаление роли\n**z/role edit** ~~-~~ изменить сообщение, отправленное Zira\n**z/role list** ~~-~~ список ролей используется\n**z/autorole** ~~-~~ дают пользователям и боты роль на соединения\n**z/log** ~~-~~ журнал, когда Zira дает/принимает роль\n\n**z/suggest** ~~-~~ представить предложения/идеи\n**z/info** ~~-~~ Информация о Zira\n**z/faq** ~~  ~~ часто задаваемые вопросы\n**z/ping** ~~-~~ Zira\'s пинг\n**z/invite** ~~-~~ Приглашаем Zira',
  ...
  faq: {
    ...
    fields: [{
      name: 'Zira не отправлять сообщения, когда я делаю любые команды',
      value: 'Убедитесь, что Zira имеет разрешение на вставлять ссылки и отправлять сообщения в канал, который вы пытаетесь команды.'
    }, {
      name: 'Zira говорит «не удается найти сообщение с этим идентификатором» или «Неизвестный идентификатор, сообщения» что делать?',
      value: 'Убедитесь, что сообщение, которое ID находится в канале вы установили, и что Зира возможность читать канала.'
    }, {
      name: 'Даже несмотря на то, что я установил его вверх и все, Zira не дает ролей.',
      value: 'Убедитесь, что роль «Zira» выше роли, которую вы хотите назначить и убедитесь, что Zira имеет разрешение на управление ролями.',
    }, {
      name: 'Zira способен удалить emojis, которые не имеют соответствующей роли установки с ними?',
      value: 'Да, позволяют Zira для управления сообщениями в канале у установки'
    }, {
      name: 'Zira способен обрабатывать более одного сообщения?',
      value: 'Да, просто установите другое сообщение и добавления ролей кэтому сообщению.'
    }, {
      name: 'Zira способен обрабатывать более одного канала?',
      value: 'Да, просто установите другой канал, а затем сообщение, и т.д.'
    }, {
      name: 'Как много ролей может быть на одно сообщение?',
      value: 'Разлад имеет максимум 20 emojis одно сообщение, так что 20 ролей'
    }]
  }
}
